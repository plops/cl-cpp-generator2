...
more structs. this function helps to initialize those structs."
  ...
  (let* ((vertex-code
	 ...
	 (code
	   `(do0
	     (comments "https://vulkan-tutorial.com/en/Drawing_a_triangle/Setup/Base_code"
...
*/"
		...
		(let ((file ("std::ifstream" filename (or "std::ios::ate"
							      "std::ios::binary"))))
		  (unless (file.is_open)
		    (throw ("std::runtime_error"
		...
		(return (logand
		 ...
		 (progn ;;defun "operator()" (vertex)
		   ...
		   (return (^
			    ("std::hash<glm::vec3>()"
			     vertex.pos)
			    (paren (>> (<< ("std::hash<glm::vec3>()"
				      vertex.color)
				     1)
				 1))
			    (paren (<< ("std::hash<glm::vec2>()"
				  vertex.texCoord)
				 1))))
		   ))))
		...
		(foreach (format availableFormats)
			 (when (logand (== VK_FORMAT_B8G8R8A8_UNORM
					format.format)
		...
		(if (!= UINT32_MAX capabilities.currentExtent.width)
		    ...
		    (do0
		     (let ((width 0)
		       ...
		       (let ((actualExtent (VkExtent2D (static_cast<uint32_t> width)
		  ...
		  (let (((queueFamilies queueFamilyCount)))
		    ...
		    (let ((i 0))
		      (foreach
		       ...
		       (when (logand (< 0 family.queueCount)
				  (and family.queueFlags
		  ...
		  (when extensionsSupported
		    (let ((swapChainSupport (querySwapChainSupport device _surface)))
		      (setf swapChainAdequate
			    (logand (not (swapChainSupport.formats.empty))
		...
		(let ((indices (findQueueFamilies device _surface))
		  ...
		  (vkGetPhysicalDeviceFeatures device
					       &supportedFeatures)
		  (return (logand (indices.isComplete)
		 ...
		 (let ((_depthImage)
		   ...
		   (let #-surface ()
		     ...
		     (defun createInstance ()
		       ...
		       #-nolog ( ;when (logand _enableValidationLayers  (not (checkValidationLayerSupport)))
			...
			(let ((formatProperties))
			  ...
			  (vkGetPhysicalDeviceFormatProperties
			   ...
			   &formatProperties)
			  (unless
			      (and formatProperties.optimalTilingFeatures
			...
			(let ((swapChainSupport
			  ...
			  (declare (type uint32_t "queueFamilyIndices[]="))
			  ...
			  (when (logand (< 0 swapChainSupport.capabilities.maxImageCount)
		 ...
		 (let ((matrix)
		   ...
		   (return 0)))))))
    (write-source *code-file* code :omit-parens t)
