...
(let ((wrappers `((:name forth_push_literal :params (vm value) :declare ((type ForthVM* vm) (type int value)) :call (vm->push_literal value))
		  (:name forth_add :params (vm) :declare ((type ForthVM* vm)) :call (vm->add))
		  (:name forth_sub :params (vm) :declare ((type ForthVM* vm)) :call (vm->sub))
		  (:name forth_mul :params (vm) :declare ((type ForthVM* vm)) :call (vm->mul))
		  ,@(loop for e in *l-prim*
			  collect
			  (destructuring-bind (&key name short &allow-other-keys) e
			    (declare (ignore name))
			    `(:name ,(format nil "forth_~a" short) :params (vm) :declare ((type ForthVM* vm)) :call (,(format nil "vm->~a" (string-downcase (format nil "~a" name)))))))
		  (:name forth_call_word :params (vm index) :declare ((type ForthVM* vm) (type int index)) :call (vm->call_word index))
		  (:name forth_pop_condition :params (vm out) :declare ((type ForthVM* vm) (type int* out)) :call (vm->pop_condition out)))))
  (write-source (format nil "~a/ForthVM_wrappers.cpp" *full-source-dir*)
		`(do0
		  (include "ForthVM.h")
		  (space extern (string "C")
			 (progn
			   ,@(loop for w in wrappers
				   collect
				   (destructuring-bind (&key name params declare call) w
				     `(defun ,name ,params
					(declare ,@declare (values int))
					(return ,call)))))))
		:format t))
