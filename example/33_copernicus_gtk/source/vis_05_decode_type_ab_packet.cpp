...
int init_decode_packet_type_a_or_b(int packet_idx,
  ...
  auto number_of_words = static_cast<int>(round(
      ceil(((static_cast<double>((((10.)) * (number_of_quads)))) / (16)))));
  auto baq_mode = ((0x1F) & ((header[37]) >> (0)));
  auto fref = (37.5347222400000000000000000000);
  auto swst = ((static_cast<double>(
                   ((((0x1) * (header[55]))) + (((0x100) * (header[54]))) +
  ...
  for (auto i = 0; (i) < (number_of_quads); (i) += (1)) {
    ...
    float scode = ((powf((-1.0f), static_cast<float>(sign_bit))) *
