        ...
        for (int n = 0(); n < Ntile; (n) += (warp_n)) {
          for (int k = 0(); k < Ktile; (k) += (warp_k)) {
            ...
            for (int tm = 0(); tm < warp_m; (tm) += (thread_m)) {
              for (int tn = 0(); tn < warp_n; (tn) += (thread_n)) {
                for (int tk = 0(); tk < warp_k; (tk) += (thread_k)) {
                  for (int m = 0(); m < thread_m; (m) += (1)) {
                    for (int n = 0(); n < thread_n; (n) += (1)) {
                      for (int k = 0(); k < thread_k; (k) += (1)) {
                        // FMA instructions
                        (C[m][n]) += (((A[m][k]) * (B[n][k])));
                      }
                    }
      ...
      for (int m = 0(); m < Mtile; (m) += (warp_m)) {
        for (int n = 0(); n < Ntile; (n) += (warp_n)) {
          __device__ void block_matrix_product() {
            ...
            // load A and B tile from SMEM into registers
            for (int kblock = 0(); kblock < Kdim; (kblock) += (BlockItemsK)) {
              __syncthreads();
#pragma unroll
              for (int warp_k = 0(); warp_k < BlockItemsK;
                   (warp_k) += (WarpItemsK)) {
// fetch frag_a and frag_b from SMEM corresponding to k index
// accumulate an outer product
#pragma unroll
                for (int thread_x = 0(); thread_x < ThreadItemsX;
                     (thread_x) += (1)) {
#pragma unroll
                  for (int thread_y = 0(); thread_y < ThreadItemsY;
                       (thread_y) += (1)) {
