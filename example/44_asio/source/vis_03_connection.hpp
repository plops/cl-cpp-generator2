...
template<typename T> class connection : public std::enable_shared_from_this<connection<T>> {
        ...
        private:
        void read_header ()    {
                boost::asio::async_read(m_socket, boost::asio::buffer(&m_msg_temporary_in.header, sizeof(message_header<T>)), [this] (std::error_code ec, std::size_t length){
                        if ( ec ) {
                                                                 
                                (std::cout)<<(std::setw(10))<<(std::chrono::high_resolution_clock::now().time_since_epoch().count())<<(" ")<<(std::this_thread::get_id())<<(" ")<<(__FILE__)<<(":")<<(__LINE__)<<(" ")<<(__func__)<<(" ")<<("read header fail")<<(" ")<<(std::setw(8))<<(" id='")<<(id)<<("'")<<(std::endl)<<(std::flush);
                m_socket.close();
} else {
        ...
        void read_body ()    {
                boost::asio::async_read(m_socket, boost::asio::buffer(m_msg_temporary_in.body.data(), m_msg_temporary_in.body.size()), [this] (std::error_code ec, std::size_t length){
                        if ( ec ) {
                                                                 
                                (std::cout)<<(std::setw(10))<<(std::chrono::high_resolution_clock::now().time_since_epoch().count())<<(" ")<<(std::this_thread::get_id())<<(" ")<<(__FILE__)<<(":")<<(__LINE__)<<(" ")<<(__func__)<<(" ")<<("read body fail")<<(" ")<<(std::setw(8))<<(" id='")<<(id)<<("'")<<(std::endl)<<(std::flush);
                m_socket.close();
} else {
                                                add_to_incoming_message_queue();
}
        ...
        void write_header ()    {
                boost::asio::async_write(m_socket, boost::asio::buffer(&m_q_messages_out.front().header, sizeof(message_header<T>)), [this] (std::error_code ec, std::size_t length){
                        if ( ec ) {
                                                                 
                                (std::cout)<<(std::setw(10))<<(std::chrono::high_resolution_clock::now().time_since_epoch().count())<<(" ")<<(std::this_thread::get_id())<<(" ")<<(__FILE__)<<(":")<<(__LINE__)<<(" ")<<(__func__)<<(" ")<<("write header fail")<<(" ")<<(std::setw(8))<<(" id='")<<(id)<<("'")<<(std::endl)<<(std::flush);
                m_socket.close();
} else {
