...
std::vector<unsigned char> forge_bench_message(
...
static void BM_ParseMultipleSmallMessages(benchmark::State& state) {
    ...
    for (int i = 0; i < num_messages; ++i) {
        std::vector<unsigned char> current_msg_bytes = forge_bench_message(
            static_cast<uint64_t>(i), NetworkProtocol::CURRENT_VERSION, small_payload);
