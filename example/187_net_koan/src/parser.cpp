...
ParseOutput parse_packet(std::vector<unsigned char>::const_iterator begin,
    ...
    if (hdr.version != NetworkProtocol::CURRENT_VERSION) {
        std::string err_msg = "Unsupported protocol version. Expected: "
                            + std::to_string(NetworkProtocol::CURRENT_VERSION)
        ...
        if (available_data_size < total_message_size_for_skip) {
            ...
            return {std::nullopt, begin, ParseResultStatus::NEED_MORE_DATA, err_msg + " Insufficient data to skip."};
        }
        ...
        invalid_msg_shell.payload.assign(begin + NetworkProtocol::Header::SIZE, begin + total_message_size_for_skip);
