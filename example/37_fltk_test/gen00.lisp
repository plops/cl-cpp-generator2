...
(progn
  (defparameter *source-dir* #P"example/37_fltk_test/source/")
	       ...
	       ;; https://www.murrayc.com/permalink/2015/07/31/gtkmm-now-uses-c11/
		    ...
		    (defun main (argc argv)
		      (declare (type int argc)
			       (type char** argv)
			       (values int))
		      ,(logprint "start" `(argc argv))
		      (let ((win (new (Fl_Window 200 200 (string "window title"))))
			    (box (new (Fl_Box 0 0 200 200 (string "hello world")))))
			(win->show)
			(return (Fl--run)))
		    )))))
