...
(progn
  (defparameter *source-dir* #P"example/47_asio_part3/source/")
		 ...
		 (_code_generation_time :type "std::string")
	      ...
	      (do0
			...
			"public client_interface<CustomMsgTypes>"
		      "public:"
		      (defmethod ping_server ()
			(let ((msg (message<CustomMsgTypes>))
			      )
			  (setf msg.header.id CustomMsgTypes--ServerPing)
			  (let ((now (std--chrono--system_clock--now)))
			    (<< msg now)
			    (send msg))))
		      (defmethod message_all ()
			(let ((msg (message<CustomMsgTypes>))
			      )
			  (setf msg.header.id CustomMsgTypes--MessageAll)
			  (send msg)))
		      )
