...
public:
    ...
    Fitab &operator=(const Fitab &other) {
        if (this != &other) {  // Check for self-assignment
            // Perform deep copy of the data members except the arrays
            a = other.a;
            b = other.b;
        }
        return *this;
...
int main(int argc, char **argv) {
    ...
    auto lin{[&](auto n, auto A, auto B, auto Sig, const auto repeat) {
...
//      }};
        ...
        auto generate_fit = [](const std::vector<Scalar> &x, const
        std::vector<Scalar> &y) -> Fitab {
            std::vector<Scalar> transformed(x.size());
            std::transform(x.begin(), x.end(), y.begin(), transformed.begin(),
                           [&](Scalar xi) {
                               return Sig * dis(gen) + A + B * xi;
                           });
            return Fitab(transformed, y);
        };


        std::mutex mtx;
        auto generate_and_store_fit = [&](int start, int end) {
            for (int i = start; i < end; ++i) {
                std::lock_guard<std::mutex> lock(mtx);
                fitres[i] = generate_fit(x, y);
            }
        };
        ...
        int num_cores = std::thread::hardware_concurrency(); // Use the number of hardware threads
        if (num_cores == 0) num_cores = 6; // Fallback to a reasonable number

        int storage_per_thread = repeat / num_cores;


        std::vector<std::jthread> workers(num_cores);
        for (int i = 0; i < num_cores; ++i) {
            int start = i * repeat / num_cores;
            int end = (i + 1) * repeat / num_cores;
            if (i == num_cores - 1) { // Ensure the last thread processes any remaining items
                end = repeat;
            }
            workers[i] = std::jthread(generate_and_store_fit, start, end);
        }
