                ...
                (let ((smax_blend (dot state x))
                  ...
                  (let (uv ro rd tVal hit p n lightPos l dif shadow objectColor col)
                    ...
                    (let ((scr_uv (/ fragCoord iResolution.xy))
                      ...
                      (let ((val (/ smax_blend 2.0f0))
                            (y_center 0.125f0)
                            (is_focused (== focused_widget 0.0f0)))
                        (declare (type float val y_center)
                                 (type bool is_focused))
                        (let ((hx (+ 0.05f0 (* val 0.35f0))))
                          (declare (type float hx))
                          (when (logand (>= scr_uv.x 0.05f0) (<= scr_uv.x 0.40f0) (< (abs (- scr_uv.y y_center)) 0.006f0))
                            (setf col (mix col (? is_focused focus_color bar_color) 0.8f0)))
                          (when (< (length (- scr_uv (vec2 hx y_center))) 0.012f0)
                            (setf col (mix col (? is_focused focus_color handle_color) 1.0f0)))))
                      
                      ;; Slider 1: shadow k factor
                      (let ((val (/ (- shadow_k 1.0f0) 99.0f0))
                            (y_center 0.205f0)
                            (is_focused (== focused_widget 1.0f0)))
                        (declare (type float val y_center)
                                 (type bool is_focused))
                        (let ((hx (+ 0.05f0 (* val 0.35f0))))
                          (declare (type float hx))
                      ...
                      (let ((val (/ (- maxDist 2.0f0) 48.0f0))
                            (y_center 0.285f0)
                            (is_focused (== focused_widget 2.0f0)))
                        (declare (type float val y_center)
                                 (type bool is_focused))
                        (let ((hx (+ 0.05f0 (* val 0.35f0))))
                          (declare (type float hx))
