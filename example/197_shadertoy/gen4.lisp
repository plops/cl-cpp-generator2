 ...
 *common-file*
 `(do0
   ...
   "// (active marker, phase, camera distance, travel ease)"
   (defun tourAux (tt)
     (declare (type float tt) (values vec4))
     (let ((seg (tourPhase tt)))
       (declare (type vec2 seg))
 ...
 *main-file*
 `(do0
   ...
   (defun markerSurface (ld active)
     (declare (type vec3 ld) (type float active) (values vec3))
     (let ((acc (vec3 0.0f0))
           (mi (vec3 0.0f0))
           (ci (vec3 0.0f0))
       ...
       ,@(loop for i from 0 below *marker-count*
               ...
               `((setf mi ,(vec3-lit (nth i (marker-dirs)))
                       ...
                       hi (? (< (abs (- active ,(f32 i))) 0.5f0) 1.0f0 0.35f0)
   ...
   (defun markerBeams (ro rd q active)
     (declare (type vec3 ro rd) (type vec4 q) (type float active) (values vec3))
     (let ((acc (vec3 0.0f0))
           (wp (vec3 0.0f0))
           (ci (vec3 0.0f0))
       ...
       ,@(loop for i from 0 below *marker-count*
               ...
               `((setf wp (qRot q ,(vec3-lit (nth i (marker-dirs))))
                       ci ,(vec3-lit (getf (nth i *markers*) :color))
                       hi (? (< (abs (- active ,(f32 i))) 0.5f0) 1.0f0 0.4f0)
   ...
   "// ---------------------------- HUD ---------------------------------"
   (defun hudOverlay (suv active phase)
     (declare (type vec2 suv) (type float active phase) (values vec3))
     (let ((acc (vec3 0.0f0))
       ...
       ,@(loop for i from 0 below *marker-count*
               ...
               `((setf c ,(vec3-lit (getf (nth i *markers*) :color))
                       ...
                       on (? (< (abs (- active ,(f32 i))) 0.5f0) 1.0f0 0.0f0)
                       dd (length (- p pc)))
       ...
       (setf play (+ 0.055f0 (* 0.045f0 (+ active phase))))
   ...
   (defun shadeSurface (ro rd tHit q qi sunDir active)
     (declare (type vec3 ro rd sunDir) (type float tHit active)
     ...
     (let ((pw (+ ro (* rd tHit)))
       ...
       (incf lit (markerSurface ld active))
       (return lit)))
   ...
   (defun mainImage (fragColor fragCoord)
     ...
     (let ((q (texelFetch iChannel0 (ivec2 0 0) 0))
           ...
           (qi (vec4 0.0f0))
           (camDist 3.0f0) (active 0.0f0) (sunA 0.0f0)
       ...
       (declare (type vec4 q aux qi clouds) (type vec2 uv)
                ...
                (type float camDist active sunA bb cc disc tHit vig))
       (setf qi (qConj q)
       ...
       (if (> tHit 0.0f0)
           (setf col (shadeSurface ro rd tHit q qi sunDir active))
       ...
       (incf col (markerBeams ro rd q active))
       ...
       (setf col (max col (vec3 0.0f0))
             ...
             col (* col vig))
       (incf col (hudOverlay (/ fragCoord iResolution.xy) active aux.y))
