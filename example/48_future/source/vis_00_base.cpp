...
int main(int argc, char **argv) {
  state._main_version = "bd4a750761c9aeb48128f1229c6bbba076e7c2ec";
  state._code_repository = "https://github.com/plops/cl-cpp-generator2/tree/"
                           "master/example/48_future";
  state._code_generation_time = "08:21:43 of Wednesday, 2020-12-02 (GMT+1)";
  state._start_time =
  ...
  result.wait();
  {
    ...
    (std::cout) << (std::setw(10))
                ...
                << (" ") << (std::this_thread::get_id()) << (" ") << (__FILE__)
                << (":") << (__LINE__) << (" ") << (__func__) << (" ") << ("")
                << (" ") << (std::setw(8)) << (" result.get()='")
                << (result.get()) << ("'") << (std::endl) << (std::flush);
  }
  return 0;
}