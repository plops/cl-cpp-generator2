...
(let ((log-preamble
  ...
  (progn
    ...
    (defparameter *source-dir* #P"example/87_pcap/source01/")
	 ...
	 ,(let ((l `(;(:name kNoiseSize :default 100 :short n)
	    ...
	    `(let ((op (popl--OptionParser (string "allowed opitons")))
		 ...
		 (exit 0))
	       ...
	       (let ((vt (std--vector<Tins--Packet>))
		     (sniffer (Tins--Sniffer (string "wlp4s0"))))
		 (while (< (vt.size)
			   10)
		   (vt.push_back
		    (sniffer.next_packet)))
		 (for-range (&packet vt)
			    (when (-> (packet.pdu)
				      (find_pdu<Tins--IP>))
			      ,(lprint :vars `((dot packet
						    (timestamp)
						    (seconds))
					       (-> (packet.pdu)
						   (dot (rfind_pdu<Tins--IP>)
							(src_addr)))))))
		 )
	...
	(macrolet ((out (fmt &rest rest)
	  ...
	  (out "pkg_check_modules( tins REQUIRED libtins )")
