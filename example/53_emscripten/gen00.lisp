		 ...
		 (let ((width 600)
		   ...
		   (let ((window (static_cast<SDL_Window*> nullptr))
		     ...
		     (SDL_CreateWindowAndRenderer width height
						  ...
						  &window
						  &renderer))
		   (space enum (curly (= MAX_ITER_COUNT 256)))
		   (let ((palette))
		     (declare (type (array SDL_Color MAX_ITER_COUNT) palette))
		     (dotimes (i MAX_ITER_COUNT)
		       (setf (aref palette i)
			     (curly (= .r (uint8_t (rand)))
				    (= .g (uint8_t (rand)))
				    (= .b (uint8_t (rand)))
				    (= .a 255))))
		     (let ((center (std--complex<double> .5d0 .5d0))
			   (scale 4d0))
		       (dotimes (y height)
			 (dotimes (x width)
			   (let ((point (std--complex<double> (/ (* 1d0 x) width)
							      (/ (* 1d0 y) height)))
				 (c (std--complex<double> (* (- point center)
							     scale)))
				 (z (std--complex<double> 0d0 0d0))
				 (i 0))
			     (for (()
				   (< i (- MAX_ITER_COUNT 1) )
				   (incf i))
