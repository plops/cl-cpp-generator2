...
inline float decode_symbol(sequential_bit_t *s) {}
void init_decode_packet(int packet_idx) {
  ...
  auto number_of_quads =
      ((((1) * (header[66]))) + (((256) * (((0xFF) & ((header[65]) >> (0)))))));
  ...
  sequential_bit_t s;
  std::array<float, number_of_quads> decoded_symbols_a;
  ...
  for (int block = 0; decoded_symbols < number_of_quads;) {
    ...
    for (int i = 0; ((i < 128) && (decoded_symbols < number_of_quads)); (i)++) {
      auto symbols_sign = (1.e+0f);
      auto symbol = decode_symbol(s);
      if (get_sequential_bit(s)) {
        symbol_sign = (-1.e+0f);
      };
      decoded_symbols_a[decoded_symbols] = ((symbol_sign));
      (decoded_symbols)++;
    };
  };
};