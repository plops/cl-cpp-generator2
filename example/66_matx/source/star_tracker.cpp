...
int main(int argc, char **argv) {
  ...
  for (auto i = 0; (i) < (size); (i) += (1)) {
    ...
    ((triangle_vertices)[(i)].position.y) =
        ((2) * (((star_data)[(i)].dec) / (90)));
    auto m{static_cast<int>(((256) / (16)) *
                            ((16) - ((2) + ((star_data)[(i)].magnitude))))};
  ...
  auto setup{[&app](Engine *engine, View *view, Scene *scene) {
    ...
    app.ib->setBuffer(
        *engine, IndexBuffer::BufferDescriptor(
                     triangle_indices, (sizeof(uint16_t)) * (118218), nullptr));
    (app.mat) =
        (Material::Builder()
  ...
  filament_app.animate([&app](Engine *engine, View *view, double now) {
    ...
    app.cam->setProjection(Camera::Projection::ORTHO, (-1) * (aspect) * (zoom),
                           (aspect) * (zoom), (-1) * (zoom), zoom, 0, 1);
