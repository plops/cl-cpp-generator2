...
-- Background colour
img:clear(Colour.new(0.02, 0.02, 0.08))

-- Simple math-based pattern
for y = 0, h - 1 do
    local ny = (y - h / 2) / (h / 2)
    for x = 0, w - 1 do
        local nx = (x - w / 2) / (w / 2)
        ...
        img:set_pixel(x, y, Colour.new(cr, cg, cb))
    end
end

-- Save with enum parameter
img:save("output.ppm", BlendMode.Add)

-- Demonstrate IntVector (std::vector<int>)
local v = make_vector()
