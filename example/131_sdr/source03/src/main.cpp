...
void DrawPlot(const MemoryMappedComplexShortFile &file, SdrManager &sdr, FFTWManager &fftw,
              const std::vector<std::vector<std::complex<double>>> &codes) {
    ...
    if (!file.ready() || (start + windowSize <= maxStart && 0 < windowSize)) {
        ...
        try {
            ...
            if (ImPlot::BeginPlot(logScale ? "FFT magnitude (dB)" : "FFT magnitude (linear)")) {
                {
                    auto pointsPerPixel = static_cast<int>(static_cast<float>(x.size()) /
...
auto initGps = [](double sampleRate, FFTWManager &fftw) {
    ...
    for (auto i = 0; i < 32; i += 1) {
        ...
        std::cout << "compute FFT" << " i='" << i << "' " << "\n" << std::flush;
        {
            ...
            std::cout << "codes" << " i='" << i << "' " << " codes.size()='" << codes.size() << "' " << " out.size()='"
                      << out.size() << "' " << "\n" << std::flush;
            codes[i] = out;
