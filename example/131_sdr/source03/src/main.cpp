...
void DrawPlot (const MemoryMappedComplexShortFile& file)        {
    ...
    auto itemsInt  = std::vector<int>({32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536, 131072, 262144, 524288, 1048576}); 
    auto windowSize  = itemsInt[windowSizeIndex]; 
    auto items  = std::vector<std::string>({"32", "64", "128", "256", "512", "1024", "2048", "4096", "8192", "16384", "32768", "65536", "131072", "262144", "524288", "1048576"}); 
    if ( ImGui::BeginCombo("Window size", items[windowSizeIndex].c_str()) ) {
        ...
        try {
            ...
            for ( auto i = 0;i<windowSize;i+=1 ) {
                                                x[i]=sampleRate*(static_cast<double>(i-(windowSize/2))/windowSize);
