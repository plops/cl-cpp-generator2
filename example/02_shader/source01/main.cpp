...
vec4 Truchet(vec2 p, vec3 col, float curve) {
  vec2 id = floor(p);
  float n = Hash21(id);
  vec3 col2 = vec3((0.));
  float depth2 = (0.);
  ...
  }
  {
    ...
    float contour = smoothstep(
        (1.00e-2), (-1.00e-2),
        ((abs(((Length(((p) + ((-0.50))), curve)) - ((0.50))))) - ((5.00e-2))));
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  vec3 col = vec3((0.));
  uv *= ((3.0));
  vec4 t1 = Truchet(uv, vec3((1.0), (0.), (0.)), (2.0));
  vec4 t2 = Truchet(((uv) + ((0.50))), vec3((0.), (1.0), (0.)), (1.0));
  if ((t2.a) < (t1.a)) {
    (col) += (t1.rgb);
  }
  if ((t1.a) < (t2.a)) {
